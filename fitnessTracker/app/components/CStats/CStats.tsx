...
export default function CStats() {
  ...
  const handleAddCalories = async () => {
    if (newCalories && !isNaN(parseFloat(newCalories))) {
      await addCaloryEntry({ date: Date.now(), calorys: parseFloat(newCalories) });
      setNewCalories('');
      setShowModal(false);
    }
  };

  const handleNumberPress = (num: string) => setNewCalories(prev => prev + num);
  const handleCommaPress = () => { if (!newCalories.includes('.')) setNewCalories(prev => prev + '.'); };
  const handleBackspace = () => setNewCalories(prev => prev.slice(0, -1));
  ...
  const renderModal = () => (
    <CustomModal
      visible={showModal}
      onClose={() => { setShowModal(false); setNewCalories(''); }}
      showCloseButton={false}
    >
      <View style={styles.modalContent}>
        <Text style={[styles.modalTitle, { color: colors.text }]}>Kalorien hinzufügen</Text>
        <View style={[styles.display, { borderColor: colors.primary }]}>
          <Text style={[styles.displayText, { color: colors.text }]}>{newCalories || '0'} kcal</Text>
        </View>
        {renderNumPad()}
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            ...
            <View style={styles.compactStats}>
              <Text style={[styles.compactText, { color: colors.text }]}>Ø {Math.round(stats.weekAverage)} kcal</Text>
